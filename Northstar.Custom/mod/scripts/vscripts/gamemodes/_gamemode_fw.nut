...
{
	// common scoreEvents
	ScoreEvent_SetEarnMeterValues( "KillHeavyTurret", 0.0, 0.2 ) // can only adds to titan's in this mode
	...
	// tower
	ScoreEvent_SetEarnMeterValues( "FortWarTowerDamage", 0.0, 0.1, 0.0 ) // using the const FW_HARVESTER_DAMAGE_SEGMENT, titans don't earn
	...
	ScoreEvent_SetEarnMeterValues( "FortWarTeamTurretControlBonus_Three", 0.0, 0.1, 0.5 )
	...
	while ( GamePlayingOrSuddenDeath() )
	{
		...
		while ( true )
		{
			...
			if ( maxSpawnCount - npcsLeft >= countPerSpawn && killsNeeded >= countPerSpawn ) // keep spawning
			{
				...
				// light units
				if ( npcToSpawn == "npc_soldier" || npcToSpawn == "npc_spectre" || npcToSpawn == "npc_stalker" )
...
const array<string> TITAN_POINT_REVERSED_MAPS = [ "mp_grave" ]
...
void function FWAreaThreatLevelThink_Threaded()
{
	...
	while ( GamePlayingOrSuddenDeath() )
	{
		...
		{
			if ( !imcEntArray.contains( titan ) && !mltEntArray.contains( titan ) && titan.GetTeam() != TEAM_IMC && !titan.e.isHotDropping )
				warnImcTitanApproach = true // this titan must be in natural space

			if ( !mltEntArray.contains( titan ) && !imcEntArray.contains( titan ) && titan.GetTeam() != TEAM_MILITIA && !titan.e.isHotDropping )
...
void function OnMegaTurretDamaged( entity turret, var damageInfo )
{
	...
	if (
		damageSourceID == eDamageSourceId.mp_titanweapon_heat_shield || damageSourceID == eDamageSourceId.mp_titanweapon_meteor_thermite ||
		damageSourceID == eDamageSourceId.mp_titanweapon_flame_wall || damageSourceID == eDamageSourceId.mp_titanability_slow_trap ||
...
void function FW_createHarvester()
{
	...
	fw_harvesterImc =
		SpawnHarvester(
			file.harvesterImc_info.GetOrigin(),
			file.harvesterImc_info.GetAngles(),
			GetCurrentPlaylistVarInt( "fw_harvester_health", FW_DEFAULT_HARVESTER_HEALTH ),
			GetCurrentPlaylistVarInt( "fw_harvester_shield", FW_DEFAULT_HARVESTER_SHIELD ),
			TEAM_IMC
		)
	...
	fw_harvesterMlt =
		SpawnHarvester(
			file.harvesterMlt_info.GetOrigin(),
			file.harvesterMlt_info.GetAngles(),
			GetCurrentPlaylistVarInt( "fw_harvester_health", FW_DEFAULT_HARVESTER_HEALTH ),
			GetCurrentPlaylistVarInt( "fw_harvester_shield", FW_DEFAULT_HARVESTER_SHIELD ),
			TEAM_MILITIA
		)
...
void function HarvesterThink( HarvesterStruct fw_harvester )
{
	...
	while ( IsAlive( harvester ) )
	{
		...
		if (
			( ( currentTime - fw_harvester.lastDamage ) >= GetCurrentPlaylistVarFloat( "fw_harvester_regen_delay", FW_DEFAULT_HARVESTER_REGEN_DELAY ) ) &&
		...
		)
		{
			...
			float newShieldHealth = (
				harvester.GetShieldHealthMax() / GetCurrentPlaylistVarFloat( "fw_harvester_regen_time", FW_DEFAULT_HARVESTER_REGEN_TIME ) * deltaTime
...
function FW_UseBattery( batteryPortvar, playervar ) // actually void function( entity, entity )
{
	...
	int newHealth = int(
		min(
			turret.GetMaxHealth(),
			turret.GetHealth() + ( turret.GetMaxHealth() * GetCurrentPlaylistVarFloat( "fw_turret_fixed_health", TURRET_FIXED_HEALTH_PERCENTAGE ) )
		)
	)
	...
	int newShield = int(
		min(
			turret.GetShieldHealthMax(),
			turret.GetShieldHealth() + ( turret.GetShieldHealthMax() * GetCurrentPlaylistVarFloat( "fw_turret_fixed_shield", TURRET_FIXED_SHIELD_PERCENTAGE ) )
		)
	)
