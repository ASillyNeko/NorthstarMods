...
void function PopulateDefaultPilotLoadouts( PilotLoadoutDef[ NUM_PERSISTENT_PILOT_LOADOUTS ] loadouts )
{
	...
	for ( int i = 0; i < NUM_PERSISTENT_PILOT_LOADOUTS; i++ )
	{
		...
		Assert(
			( loadout.primaryMod1 == "" && loadout.primaryMod2 == "" ) || ( loadout.primaryMod1 != loadout.primaryMod2 ),
			"!!! Primary mod1 and mod2 in default pilot loadout: " + loadout.name + " should be different but are both set to: " + loadout.primaryMod1
		)
		...
		Assert(
			( loadout.secondaryMod1 == "" && loadout.secondaryMod2 == "" ) || ( loadout.secondaryMod1 != loadout.secondaryMod2 ),
			"!!! Secondary mod1 and mod2 in default pilot loadout: " + loadout.name + " should be different but are both set to: " + loadout.secondaryMod1
		)
		...
		Assert(
			( loadout.weapon3Mod1 == "" && loadout.weapon3Mod2 == "" ) || ( loadout.weapon3Mod1 != loadout.weapon3Mod2 ),
			"!!! Weapon3 mod1 and mod2 in default pilot loadout: " + loadout.name + " should be different but are both set to: " + loadout.weapon3Mod1
		)
...
string function GetPersistentLoadoutPropertyType( string loadoutProperty )
{
	...
	{
		// printt( "=======================================================================================" )
		// printt( "SetPersistentLoadoutValue called with loadoutType:", loadoutType,
		// "loadoutIndex:", loadoutIndex, "loadoutProperty:" , loadoutProperty, "value:", value )
		// printl( "script GetPlayerArray()[0].SetPersistentVar( \"" + loadoutType + "Loadouts[" + loadoutIndex + "]." + loadoutProperty + "\", \"" + value + "\" )" )
		// printt( "=======================================================================================" )
		...
		// Assert( ( loadoutIsPilot && IsValidPilotLoadoutIndex( loadoutIndex ) ) || ( loadoutIsTitan && !IsValidTitanLoadoutIndex(
		// loadoutIndex ) ), "SetPersistentLoadoutValue() called with invalid loadoutIndex: " + loadoutIndex )
		...
		if ( LoadoutPropertyRequiresItemValidation( loadoutProperty ) && value != "" )
		{
			if ( FailsLoadoutValidationCheck( player, loadoutType, loadoutIndex, loadoutProperty, value ) )
			{
				printt( "=======================================================================================" )
				printt(
					"SetPersistentLoadoutValue failed FailsLoadoutValidationCheck with loadoutType:",
					loadoutType,
					"loadoutIndex:",
					loadoutIndex,
					"loadoutProperty:",
				...
				// Assert( false, "SetPersistentLoadoutValue() call " + loadoutType + " " + loadoutProperty
				// + ", value: " + value + " failed FailsLoadoutValidationCheck() check!" )
				return
			}
		}
		...
		{
			CodeWarning(
				loadoutType + " " + loadoutProperty + " value: " + value +
					" failed IsLoadoutSubitemValid() check! It may not exist in itemData, or it's parent item is locked."
			)
			return
		}
		...
		bool updateModel = (
			loadoutProperty == "suit" || loadoutProperty == "race" || loadoutProperty == "setFile" || loadoutProperty == "primary" || loadoutProperty == "isPrime"
		)
	...
	{
		// printt( "FailsLoadoutValidationCheck, player: " + player + ", loadoutType: " + loadoutType + ",
		// loadoutIndex: " + loadoutIndex + ", loadoutProperty: " + loadoutProperty + ", value: " + value )
		...
		if ( !IsRefValid( ref ) )
		{
			#if DEV
				printt(
					"!IsRefValid( " + ref + " ), generated from loadoutType: " + loadoutType + ", loadoutIndex: " + loadoutIndex + ". loadoutProperty: " + loadoutProperty +
						", value: " + value
				)
			#endif
		...
		{
			// printt( "!IsValueValidForLoadoutTypeIndexAndProperty, loadoutType: " + loadoutType + ", loadoutIndex:
			// " + loadoutIndex + ", loadoutProperty: " + loadoutProperty + ", value: " + value + " ref: " + ref )
			return true
		}

		if (
			( loadoutProperty == "secondary" || loadoutProperty == "weapon3" ) &&
			!IsValueValidForPropertyWithCategoryRestriction( player, loadoutIndex, loadoutProperty, string( value ) )
		)
			return true
	...
	bool function ValidateSkinAndCamoIndexesAsAPair( entity player, string loadoutType, int loadoutIndex, string loadoutProperty, var value )
	{
		...
		// printt( "ValidateSkinAndCamoIndexesAsAPair, player: " + player + " loadouType: " + loadoutType + ", loadoutIndex: " + loadoutIndex +  ",
		// skinProperty: " + loadoutProperty +  ", skin Value: "  + string( value ) + ", camoProperty: " + camoProperty + ", camo Value: " + camoIndexValue
		// )
	...
	bool function SetCamoIndexToValue( entity player, string loadoutType, int loadoutIndex, string camoProperty, int setValue ) // HACK HACK: should fix dependency between skin and camo index on UI side. Remove when done.
	{
		...
		// printt( "SetCamoIndexToValue, player: " + player + " loadouType: " + loadoutType + ", loadoutIndex:
		// " + loadoutIndex + ", camoProperty: " + camoProperty + ", setValue: "  + string( setValue ))
...
string function GetLoadoutPropertyDefault( string loadoutType, int loadoutIndex, string propertyName ) // Bad things go wrong if you give the wrong default!
{
	...
	// printt( "GetLoadoutPropertyDefault, loadoutType: " + loadoutType + ", loadoutIndex: " +
	// loadoutIndex + ", propertyName: " + propertyName + ", resultString: " + resultString )
	return resultString
}
...
void function ResolveInvalidLoadoutChildValues( entity player, string loadoutType, int loadoutIndex, string loadoutProperty, string parentValue )
{
	...
	if ( childProperties.len() )
	{
		...
		foreach ( childProperty in childProperties )
		{
			...
			{
				#if SERVER
					// TODO: This call will actually check and change the persistent data if it's invalid.
					// Doesn't break anything here, but this function really shouldn't change persistent data.
...
string function Loadouts_GetSetFileForRequestedClass( entity player )
{
	...
	bool function Loadouts_TryGivePilotLoadout( entity player )
	{
		...
		if ( GetCurrentPlaylistVarInt( "custom_pilot_loadout", 0 ) )
		{
			...
			loadout.primaryMods =
				[
					GetCurrentPlaylistVarString( "pilot_loadout_primary_attachment", "" ),
					GetCurrentPlaylistVarString( "pilot_loadout_primary_mod1", "" ),
					GetCurrentPlaylistVarString( "pilot_loadout_primary_mod2", "" ),
					GetCurrentPlaylistVarString( "pilot_loadout_primary_mod3", "" )
				]
			...
			loadout.secondaryMods =
				[
					GetCurrentPlaylistVarString( "pilot_loadout_secondary_mod1", "" ),
					GetCurrentPlaylistVarString( "pilot_loadout_secondary_mod2", "" ),
					GetCurrentPlaylistVarString( "pilot_loadout_secondary_mod3", "" )
				]

			loadout.weapon3 = GetCurrentPlaylistVarString( "pilot_loadout_weapon3", "" )
			loadout.weapon3Mods =
				[
					GetCurrentPlaylistVarString( "pilot_loadout_weapon3_mod1", "" ),
					GetCurrentPlaylistVarString( "pilot_loadout_weapon3_mod2", "" ),
					GetCurrentPlaylistVarString( "pilot_loadout_weapon3_mod3", "" )
				]
...
void function PrintTitanLoadout( TitanLoadoutDef loadout )
{
	...
	printt( "        primarySkinIndex     " + loadout.primarySkinIndex )
	printt( "        primaryCamoIndex     " + loadout.primaryCamoIndex )
	printt( "        titanExecution       \"" + loadout.titanExecution + "\"" )
...
int function GetSkinIndexForCamo( string modelType, string camoPropertyName, int camoIndex )
{
	...
	Assert(
		camoPropertyName == "camoIndex" || camoPropertyName == "primeCamoIndex" || camoPropertyName == "primaryCamoIndex" || camoPropertyName == "secondaryCamoIndex"
			|| camoPropertyName == "weapon3CamoIndex"
	)
