...
const array<string> SWAP_MAPS = [ "mp_forwardbase_kodai", "mp_lf_meadow" ]
...
void function CaptureTheFlag_Init()
{
	...
	AddSpawnCallback( "info_spawnpoint_dropship_start", SwapTeam )
	AddSpawnCallback( "info_spawnpoint_titan_start", SwapTeam )
	AddSpawnCallback( "info_spawnpoint_human_start", SwapTeam )
	...
	ScoreEvent_SetEarnMeterValues( "KillPilot", 0.05, 0.2 )
	ScoreEvent_SetEarnMeterValues( "PilotAssist", 0.05, 0.1 )
	...
	ScoreEvent_SetEarnMeterValues( "FlagCarrierKill", 0.0, 0.2 )
	ScoreEvent_SetEarnMeterValues( "FlagTaken", 0.0, 0.1 )
	ScoreEvent_SetEarnMeterValues( "FlagCapture", 0.0, 0.3 )
	ScoreEvent_SetEarnMeterValues( "FlagCaptureAssist", 0.0, 0.2 )
	ScoreEvent_SetEarnMeterValues( "FlagReturn", 0.0, 0.2 )
}
...
void function SwapTeam( entity spawnPoint )
{
	if ( SWAP_MAPS.contains( GetMapName() ) )
...
{
	if (
		!IsAlive( player ) || flag.GetParent() != null || FlagIngoresPlayerTitans( player ) || player.IsPhaseShifted() || player.p.isEmbarking ||
		player.p.isDisembarking || player.p.pilotEjecting
	)
		return false
...
void function CaptureFlag( entity player, entity flag )
{
	...
	SetRoundWinningKillReplayEntities( player, null, -1 ) // set attacker for last cap replay
